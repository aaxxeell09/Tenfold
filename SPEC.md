...
F10. Added on 2026-09-13, rewritten the same evening. A course node does not shape what a session serves at all: the node name is a label on the screen. Every question is drawn uniformly at random from the fifteen facts of 6 to 10 in both orientations, with no fact asked twice in a session unless it was answered wrong, never the same table twice in a row, and orientation alternating. `MAX_OUTSIDE_REVIEWS` and the whole idea of a node scope steering the draw are gone. The first version of this amendment kept the node's pairs first and widened after them; that lasted an hour and was replaced because it was still a rule to reason about for no gain the child could feel. The scripted demo scenario is unaffected and still plays fixed under `--demo`.

F11. Added on 2026-09-13. The check is a ready gate at the start of every activity, not a one time onboarding. Step 1 both hands in frame, step 2 ready, and the pose step only the very first time a child plays; the last step is a microphone test, "Say: I'm ready!", with no arithmetic in it. It takes under ten seconds, holds `gate_step_pause_ms` between steps, offers a Ready button after `gate_ready_button_s` without recognition, and can be skipped by a tap on Tally once a gate has passed in the same session. The check node is an empty node: `app/server.py` draws no exercise for it, records no outcome, writes no live sample and ends it on the page's `ready` or `quit`; when the gate names a pair, the engine is armed on it so the verdict reaches the page, and nothing scores it. Its lines are the page's, from `lesson/tally_lines.json`.

F12. Added on 2026-09-13. The intervention ladder shows before it speaks. L0 observes, silent. L1, after `idle_nudge`, puts the finger numbers up, no voice. L2, after `hint_2_delay`, colours the two fingers that matter, no voice. L3 speaks the correction and moves the ghost finger. L4 is the rescue as before, outside the unsolicited cap. Contact is read from the fingertips: the two tips the classifier names have to be within `contact_ratio` of a palm width (default 0.35, bounds [0.2, 0.6]) for `pose_confirm_frames` frames before the pose counts as made, on top of the classifier's own flag, which `classifier/rules.py` keeps and nobody else edits. On the tutor's side F8 holds too: the correct pose is confirmed in `pose_confirm_frames` frames or `pose_confirm_ms`, whichever first; a wrong pose still waits `pose_stable` before it is corrected, so a hand passing through a wrong shape on its way is not told off. Hands back in frame get `recovery_grace_ms` before anything is judged, and two corrections are never closer than `post_line_grace_ms`. The success beat between exercises, `success_beat_ms` then `next_pause_ms` then "Next one.", is never skipped, in any mode.
