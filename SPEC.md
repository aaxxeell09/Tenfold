...
2. Trois appels `claude -p` distincts, chacun tracé par un `@weave.op` (prompt + résumé de sortie, jamais l'environnement) :
   ...
   - **Patch** : `--allowedTools "Read,Edit,Bash(python loop/guard.py --check),Bash(python loop/smoke.py),mcp__wandb__*" --max-turns 30`, timeout 10 minutes. Une hypothèse, un patch cohérent, une justification, un effet attendu. Le prompt énonce le critère de succès (exact_match train qui monte, false_unknown_rate sous 5 %) et la checklist exacte de la garde.
...
4. `run_eval.py --split train`, puis **porte métrique** : le patch est accepté seulement si `exact_match` train ne baisse pas et qu'aucune classe ne perd plus de 5 points. Sinon le patch est jeté et le rejet est loggé comme op Weave.
